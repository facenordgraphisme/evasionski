...
async function fixGuideId() {
  console.log('🔧 Correction de l\'ID du document guide...\n')

  const oldId = 'VBJKCq85QlNgxho2iUCXI3'
  const correctId = 'guide'

  // 1. Supprimer les anciens documents s'ils existent
  console.log('1️⃣ Nettoyage...')
  try {
    await client.delete(oldId)
    console.log('   ✅ Ancien document (ID incorrect) supprimé')
  } catch (error) {
    console.log('   ℹ️  Pas d\'ancien document avec ID incorrect')
  }

  try {
    await client.delete(correctId)
    console.log('   ✅ Document existant (ID correct) supprimé')
  } catch (error) {
    console.log('   ℹ️  Pas de document existant avec ID correct')
  }

  // Supprimer aussi le draft s'il existe
  try {
    await client.delete(`drafts.${correctId}`)
    console.log('   ✅ Draft supprimé')
  } catch (error) {
    console.log('   ℹ️  Pas de draft')
  }

  // 2. Créer un nouveau document avec le BON ID
  console.log('\n2️⃣ Création du document avec le bon ID...')
  ...
  try {
    ...
    console.log('   ✅ Document créé avec le bon ID:', result._id)
    console.log('\n✨ SUCCÈS ! Rafraîchis le Studio et teste maintenant.')
  } catch (error: any) {
    console.error('   ❌ Erreur:', error.message)
  }
}

fixGuideId().catch(console.error)
