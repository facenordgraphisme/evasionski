...
export default async function BlogDetailView({ post }: BlogDetailViewProps) {
              ...
              <div className="flex flex-wrap gap-2.5 mb-8">
                {post.tags.map((tag: any, idx: number) => {
                  const href = tag.tagType === 'massif' 
                    ? `/blog-explorez-les-hautes-alpes-a-ski?massif=${tag.slug}` 
                    : `/blog-explorez-les-hautes-alpes-a-ski?category=${tag.slug}`;
                  return (
                    <Link 
                      key={idx} 
                      href={href}
