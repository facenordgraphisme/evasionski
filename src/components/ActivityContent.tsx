...
const ActivityContent = ({ data, initialLang = 'fr' }: ActivityContentProps) => {
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
            <div className="lg:col-span-2 space-y-12">
              <div>
                <h2 className="text-3xl font-bold mb-6">{t('activity.presentation')}</h2>
            ...
            <div className="lg:col-span-1">
              <div className="glass p-10 rounded-[40px] sticky top-32">
                <h3 className="text-2xl font-bold mb-6">{t('activity.informations')}</h3>
                <div className="space-y-6 mb-10">
                  <div className="flex justify-between items-center py-4 border-b border-white/10">
                    <span className="text-foreground/40 font-bold uppercase tracking-widest text-xs">{t('activity.price')}</span>
                    <span className="text-xl font-bold text-highlight">{at(data.price)}</span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-white/10">
                    <span className="text-foreground/40 font-bold uppercase tracking-widest text-xs">{t('activity.period')}</span>
                    <span className="font-bold text-white">{at(data.period) || t('activity.seasonal')}</span>
                  </div>
                  <div className="flex justify-between items-center py-4 border-b border-white/10">
                    <span className="text-foreground/40 font-bold uppercase tracking-widest text-xs">{t('activity.location')}</span>
                    <span className="font-bold text-white">{at(data.location) || t('activity.defaultLocation')}</span>
                  </div>
                </div>
                <Link href="/evasion-ski-hautes-alpes-contact" className="btn-primary w-full block text-center !text-white">{t('activity.book')}</Link>
        ...
        <section className="py-24 bg-accent/5">
          <div className="container mx-auto px-6">
            <div className="text-center mb-16 max-w-3xl mx-auto">
              <span className="text-accent font-black tracking-widest uppercase text-xs mb-4 block">{at(data.universBadge) || t('activity.universBadge')}</span>
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase mb-6">{at(data.universTitle) || t('activity.universTitle')}</h2>
