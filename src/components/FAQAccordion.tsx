...
const DEFAULT_FAQS = [
  {
    _id: 'default-1',
    question: "Quel niveau physique et technique faut-il pour débuter en ski de randonnée ?",
    questionEn: "What physical and technical level is required to start ski touring?",
    answer: "Pour vous initier au ski de randonnée, il faut être capable de descendre une piste rouge en contrôle. Physiquement, une condition générale correcte suffit pour envisager 3 à 4 heures de montée lente (environ 400 à 600m de dénivelé). Le matériel spécifique (peaux de phoque, conversions) et la sécurité vous seront expliqués par votre moniteur lors de la sortie.",
    answerEn: "To learn ski touring, you should be able to control your descent on a red slope. Physically, a decent general condition is sufficient for 3 to 4 hours of slow climbing (around 400 to 600m elevation gain). The specific gear (skins, kick turns) and safety instructions will be explained by your instructor during the outing.",
    category: 'technical'
  },
  {
    _id: 'default-2',
    ...
    answer: "La réservation s'effectue par email ou via le formulaire de contact. Des arrhes de 30% sont demandées pour valider définitivement la date. Le solde est réglé directement au moniteur au début ou à la fin de la sortie (chèque, espèces ou virement). Les frais de refuge et de remontées mécaniques restent à votre charge directe.",
    answerEn: "Bookings can be made by email or through the contact form. A 30% deposit is required to secure the date. The balance is paid directly to the instructor at the start or end of the outing (check, cash, or bank transfer). Mountain hut fees and ski lift passes are paid directly by you.",
    category: 'general'
  },
  {
    _id: 'default-3',
    ...
    answer: "La sécurité est la priorité absolue. En cas de météo défavorable ou de conditions de montagne dangereuses (risque d'avalanche élevé), le moniteur vous proposera un itinéraire de repli adapté, une autre activité ou un report de la sortie. Si aucune alternative n'est possible, les arrhes vous sont remboursées.",
    answerEn: "Safety is our absolute priority. In case of unfavorable weather or dangerous mountain conditions (high avalanche risk), the instructor will suggest an alternative route, a different activity, or a postponement. If no alternative is possible, the deposit is refunded.",
    category: 'safety'
  }
]
            ...
            return (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-semibold tracking-wide border transition-all duration-300 ${
                  isActive
                    ? 'bg-accent border-accent text-slate-900 shadow-[0_0_15px_rgba(243,177,8,0.3)]'
                    : 'bg-foreground/5 border-border text-foreground/60 hover:text-accent hover:border-accent/30'
                }`}
              >
                {label}
              </button>
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-6 px-6 md:px-8 py-5 text-left text-lg md:text-xl font-bold text-foreground transition-colors duration-200"
                  >
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="px-6 md:px-8 pb-6 text-base md:text-lg text-foreground/80 leading-relaxed border-t border-border pt-4">
                          <p>{answer}</p>
                        </div>
                      </motion.div>
                    )}
