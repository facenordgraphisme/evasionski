...
export default function SortiesFilterableList({ initialSorties }: SortiesFilterableListProps) {
  ...
  const getActivityColor = (type: string) => {
    const map: Record<string, string> = {
      'ski-de-randonnee-engagement-prive': 'bg-accent',
      'ski-randonnee-hautes-alpes-journee': 'bg-secondary',
      'ski-hors-piste-station-hautes-alpes': 'bg-highlight',
      'stages-et-raids-a-ski-de-randonnee-hautes-alpes': 'bg-purple-500'
    }
    return map[type.toLowerCase()] || 'bg-foreground/20'
  }
  ...
  const getCategoryLabel = (type: string) => {
    const map: Record<string, string> = {
      'ski-de-randonnee-engagement-prive': at('Engagement Privé'),
      'ski-randonnee-hautes-alpes-journee': at('Ski de rando journée'),
      'ski-hors-piste-station-hautes-alpes': at('Freerando'),
      'stages-et-raids-a-ski-de-randonnee-hautes-alpes': at('Stages & Raids')
    }
    return map[type.toLowerCase()] || type
  }
        ...
        <div className="sticky top-32 space-y-10">
          ...
          <div className="space-y-6 px-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { name: at('Engagement Privé'), color: 'bg-accent' },
                { name: at('Ski de rando journée'), color: 'bg-secondary' },
                { name: at('Freerando'), color: 'bg-highlight' },
                { name: at('Stages & Raids'), color: 'bg-purple-500' }
              ].map(cat => (
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-[10px] font-black transition-all uppercase tracking-widest border ${
                  filter === cat
                    ? 'bg-accent border-accent text-slate-900 shadow-xl scale-105'
            ...
            {visibleSorties.length > 0 ? (
              visibleSorties.map((s, index) => (
                <motion.div
                  ...
                  className={`group glass-card rounded-[40px] overflow-hidden flex flex-col border shadow-xl ${
                    s.isPassed 
