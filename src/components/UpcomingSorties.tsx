...
const UpcomingSorties = ({ 
  ...
  const filteredSorties = safeData.filter(s => {
    ...
    const matchesCat = (() => {
      ...
      if (filter === 'Engagement Privé' || filter === at('Engagement Privé')) {
        return s.sejour.activityType === 'ski-de-randonnee-engagement-prive';
      }
      if (filter === 'Ski de rando journée' || filter === at('Ski de rando journée')) {
        return s.sejour.activityType === 'ski-randonnee-hautes-alpes-journee';
      }
      if (filter === 'Freerando' || filter === at('Freerando')) {
        return s.sejour.activityType === 'ski-hors-piste-station-hautes-alpes';
      }
      if (filter === 'Stages & Raids' || filter === at('Stages & Raids')) {
        return s.sejour.activityType === 'stages-et-raids-a-ski-de-randonnee-hautes-alpes';
      }
      return s.sejour.activityType.toLowerCase().includes(filter.toLowerCase());
    })();
